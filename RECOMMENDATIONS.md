...
Claude Code のMCPサーバーの設定は、3つの異なるスコープレベルで設定することが可能です。
詳細についてはドキュメントを参照してください: https://code.claude.com/docs/ja/mcp#mcp-インストールスコープ

プロジェクトを問わず使用するMCPサーバーは、ユーザースコープに設定すると便利です。設定はコマンドで追加するか、`~/.claude.json` を直接編集します。

jsonファイルの記述例は下記のとおりです。
```json
{
  "mcpServers": {
    "backlog": {
